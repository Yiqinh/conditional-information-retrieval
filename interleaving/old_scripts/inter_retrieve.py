...
if __name__ == "__main__":
    ...
    f = os.path.join(here, 'v2_search_res', 'v2_search_test_prompt1_all.json')
    already_seen_docs = set()
    ...
    with open(f, 'r') as file:
        ...
        for article in articles:
            for source in article['sources']:
                id = article['url'] + "#" + source['Name']
                if id not in already_seen_docs:
                    included_documents.append(id)


    res = []

    with open(os.path.join(here, 'queries', 'query2_v2_parsed.json'), 'r') as file:
        url_to_new_query = json.load(file)

    #get search queries
    with open(f, 'r') as file:
        articles = json.load(file)
        for article in tqdm(articles, desc="article source search"):
            my_query = url_to_new_query['url']
    ...
    fname = os.path.join(here, 'v2_search_res', 'v2_search_test_prompt1_all_inter2.json')
